...
namespace Tida.Geometry.Alternation
{
    ...
    public class Matrix
    {
        ...
        public double this[int row, int col]
        {
            get
            {
                return elements[col + row * numColumns];//似乎应该是row-1 和col-1吧
            }
            set
            {
        ...
        public Matrix(double[,] value)
        {
            ...
            for (int i = 0; i < numRows; ++i)
            {
                ...
                {
                    data[k++] = value[i, j];//二维数组的值都添加到一维数组中
                }
            }
            Init(numRows, numColumns);
            SetData(data);
        ...
        public Matrix Subtract(Matrix other)
        {
            ...
            {
                for (int j = 0; j < numColumns; ++j)
                    result.SetElement(i, j, result.GetElement(i, j) - other.GetElement(i, j));//获得矩阵中元素的值和设置矩阵中元素的值都是在一维数组查询值和获得值
            }
        ...
        public Matrix Multiply(Matrix other)
        {
            ...
            for (int i = 0; i < result.GetNumRows(); ++i)
            {
                for (int j = 0; j < other.GetNumColumns(); ++j)
                {
                    ...
                    {
                        value += GetElement(i, k) * other.GetElement(k, j);//对应元素相乘求和
                    }
